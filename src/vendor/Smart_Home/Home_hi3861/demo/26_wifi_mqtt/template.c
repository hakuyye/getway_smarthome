...
#define SERVER_IP_ADDR "183.230.40.96"    // mqtts.heclouds.com
#define SERVER_IP_PORT 1883
#define MQTT_TOPIC_SUB "$sys/E1e65BBc1u/hi3861/thing/property/set"//订阅主题
#define MQTT_TOPIC_PUB "$sys/E1e65BBc1u/hi3861/thing/property/post"//发布主题

#define TASK_INIT_TIME 2 // s
#define MQTT_RECV_TASK_TIME (200 * 1000) // us

#define CLIENTID "hi3861"
#define USERNAME "E1e65BBc1u"
#define PASSWORD "version=2018-10-31&res=products%2FE1e65BBc1u%2Fdevices%2Fhi3861&et=1771915335&method=md5&sign=SUrHqVD2HvwMys5nsdcRiw%3D%3D"
...
void mqtt_recv_task(void)//接收mqtt消息任务
{
    while (1) 
    {
        MQTTClient_sub();
...
void mqtt_send_task(void)
{
    ...
    // 连接MQTT客户端
    if (MQTTClient_init(CLIENTID, USERNAME, PASSWORD) != 0) 
    {
    ...
    while (1) 
    {
        ...
        float temperature = 34.56;
        unsigned char str[200];
        sprintf(str,"{\"id\":\"1740384608345\",\"version\":\"1.0\",\"params\":{\"Humidity\":{\"value\":%.2f},\"Temperature\":{\"value\":%.2f}}}",humidity,temperature);
        MQTTClient_pub(MQTT_TOPIC_PUB,str, strlen(str));//发布消息
        sleep(TASK_INIT_TIME);
    }
}
//任务创建
