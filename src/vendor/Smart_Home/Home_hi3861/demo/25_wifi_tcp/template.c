...
void WIFI_Task(void)
{
    ...
    // 连接服务器
    re = connect(socket_fd, (struct sockaddr *)&addr, sizeof(addr)); //------------
    if (re == -1) 
    {
    ...
    while (1) 
    {
        memset_s(buff, sizeof(buff), 0, sizeof(buff));//---------清空缓冲区

        //	接收客户端发送过来的消息
        re = recv(socket_fd, buff, sizeof(buff), 0); //----------------------------
        if (re <= 0) 
        {
            break;
        } 
        else 
        {
            ...
            send(socket_fd, buff, sizeof(buff), 0);//-------------把接受的消息发送回去
        }
    }
