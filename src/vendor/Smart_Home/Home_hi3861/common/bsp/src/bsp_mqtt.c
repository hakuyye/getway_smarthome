...
/**
 * @brief 设置MQTT客户端服务器:创建套接字-连接TCP服务器
 * @param ip_addr:服务器IP地址
 * @param ip_port:服务器端口号
 * @return 0:成功, -1:失败
 * */ 
...
/**
 * @brief 订阅MQTT主题:序列化订阅主题信息-发送订阅包-接收订阅确认包
 * @param subTopic:订阅主题
 * @return 0:成功, -1:失败
 * */
int MQTTClient_subscribe(char *subTopic)
{
    ...
    res = transport_sendPacketBuffer(mqttBuff, len);//发送数据包
    if (res != 1) {
    ...
    /* wait for suback */
    if (MQTTPacket_read(mqttBuff, sizeof(mqttBuff), transport_getdata) != SUBACK) {//接收响应包
...
// 保持在线时长 60s
#define MQTT_KEEP_ALIVE 1800
#define MQTT_DELAY_TIME 3

/**
 * @brief 连接MQTT客户端：配置连接客户端的信息-序列化头部信息-发送connect包-接收connack包
...
int MQTTClient_init(char *clientID, char *userName, char *password)
{
    ...
    // 组MQTT消息包
    len = MQTTSerialize_connect(mqttBuff, sizeof(mqttBuff), &mqttData);//序列化,把结构体mqttData转化为字节流mqttBuff
    if (len <= 0) {
    ...
    if (MQTTDeserialize_connack(&sessionPresent, &connack_rc, mqttBuff, sizeof(mqttBuff)) != 1 || connack_rc != 0) {//反序列化,把字节流mqttBuff分解出来
...
/**
 * 
 * @brief 发布MQTT消息：序列化发布主题信息-发送publish包-接收puback包
 * @param pub_Topic:发布主题
 * @param payloadData:发布消息
 * @param payloadLen:发布消息长度    
 * @return 0:成功, -1:失败
*/
