...
#define SELECT_WLAN_PORT "wlan0"//无线网络接口，一般为wlan0；有线网络接口，一般为eth0
...
/**
 * @brief  创建热点:使能WiFi-判断WiFi是否激活-设置指定的热点信息-开启WiFi热点模式-检查WiFi热点是否激活
 * @param ssid: AP 的名称
 * @param psk:AP 的密码
 * @retval WIFI_SUCCESS-成功，其他-失败
 */
...
/**
 * @brief  连接指定的WiFi热点:使能WiFi-判断WiFi是否激活-注册WiFi连接状态改变的回调函数-连接指定的WiFi热点-开启DHCP-获取IP地址
 * @note
 * @param ssid: AP的名称
 * @param psk: AP的密码
 * @retval WIFI_SUCCESS-成功，其他-失败
...
{
    WifiErrorCode result;
    int Timeout = 10; // 超时时间 10s，DHCP获取IP地址的等待时就
    ...
    result = AddDeviceConfig(&wifiDeviceConfig, &wifiResult);//---------------------------配置WiFi热点信息
     if ((result == WIFI_SUCCESS) && (ConnectTo(wifiResult) == WIFI_SUCCESS)&& (WaitConnectResult() == 1)) {//------------------连接WiFi，且没有超时
        printf("wifi connect succeed!.\r\n");
        g_lwip_netif = netifapi_netif_find(SELECT_WLAN_PORT);//------------------查找特定的网络接口
        //启动DHCP
        if (g_lwip_netif) {
            dhcp_start(g_lwip_netif);//---------------------------启动DHCP，用于自动分配 IP 地址和其他网络配置信息（如子网掩码、默认网关和 DNS 服务器）
        }

        //等待DHCP
        for (;;) {
            if (dhcp_is_bound(g_lwip_netif) == ERR_OK) {
                Sta_GetWiFiIP(g_lwip_netif, g_IP_Addr);//----------------------获取IP地址
